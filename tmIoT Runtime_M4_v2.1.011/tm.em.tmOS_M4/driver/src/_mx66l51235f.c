...
+------------------------------------------------------------------------------*/
ERROR_T     _Mx66151235f_Open     (   void      *pArg   );
SIZE_T      _Mx66151235f_Read     (  BYTE_T      *pbyBuf,
                                    SIZE_T      Size, 
                                    OFFSET_T    Offset  );
SIZE_T      _Mx66151235f_Write     ( BYTE_T      *pbyBuf,
                                     SIZE_T      wSize, 
                                     OFFSET_T    dwAddr  );
/*------------------------------------------------------------------------------+
|                    Globle Variable                                            |
+------------------------------------------------------------------------------*/
DEV_ENGINE_T            g_Mx66151235f_Engine  =
{
    _Mx66151235f_Open,
    NULL,
    _Mx66151235f_Read,
...
SIZE_T      _Mx66151235f_Read     (  BYTE_T      *pbyBuf,
                                    ...
                                    OFFSET_T    Offset  )
{
    if (Size + Offset > MX66L51235F_MEMORY_SIZE )  
        return 0;

    MX66L51235FRead(Offset, pbyBuf, Size);
    return  0;
}
...
{
    WORD    wWriteSize = 0; 
    DWORD   dwEraseAddr = dwAddr;  
    
    if (wSize + dwAddr > MX66L51235F_MEMORY_SIZE)
        return  0;
    //
    //Flash写入数据时，必须先擦数.
    //flash的特性是，写数据只能将1写为0，0不能写为1.擦除数据是将所有数据都写为1.
    //因此如果想在已经写过数据的flash上写入新的数据，则必须先擦除。
    //
    if((dwEraseAddr & MX66L51235F_BLOCK_MARK) + wSize <= MX66L51235F_BLOCK_SIZE)
    {
        MX66L51235FSectorErase(dwEraseAddr);
    }
    else
    {
        MX66L51235FSectorErase(dwEraseAddr);
        dwEraseAddr = dwEraseAddr + (MX66L51235F_BLOCK_SIZE - (dwEraseAddr & MX66L51235F_BLOCK_MARK));
        while(dwEraseAddr < dwAddr + wSize)
        {
            MX66L51235FSectorErase(dwEraseAddr);
            dwEraseAddr += MX66L51235F_BLOCK_SIZE;
        }
    }

    //如果从dwAddr地址开始，wSize的数据不超过1页(256 BYTE)
    if((dwAddr & MX66L51235F_PAGE_MARK) + wSize <= MX66L51235F_PAGE_SIZE)
    {
        MX66L51235FPageProgram(dwAddr, pbyBuf, wSize);  
    }
    else
    {   
        //
        //分页(256BYTE),循环将数据写入Flash中.
        //
        while((dwAddr & MX66L51235F_PAGE_MARK) + wSize > MX66L51235F_PAGE_SIZE)
        {
            wWriteSize = MX66L51235F_PAGE_SIZE - (dwAddr & MX66L51235F_PAGE_MARK);
            MX66L51235FPageProgram(dwAddr, pbyBuf, wWriteSize); 
            pbyBuf += wWriteSize;
            dwAddr += wWriteSize;
            wSize  -= wWriteSize;
        }
